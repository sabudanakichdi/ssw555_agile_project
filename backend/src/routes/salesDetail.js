...
router.put('/', auth, [
    ...
    try {
        var checkUser;
        console.log
        await SalesDetail.findOne({ customer_id: customer_id }).then(result => {
...
router.get('/', auth, async (req, res) => {
    try {
        var customerDb;
        await Customer.findOne({ _id: req.body.customer_id }).then(result => {
            if (result) {
        ...
        var order;
        await OrderTracking.findOne({ customer_id: customerDb._id }).then(result => {
            if (result) {
                console.log(result); // Found a matching document
                order = result
            } else {
