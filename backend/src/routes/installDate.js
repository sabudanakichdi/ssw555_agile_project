...
router.put(
  ...
  auth,
  [
    check("install_date", "Please enter installation date").isLength({
      min: 1,
    }),
  ],
  async (req, res) => {
    ...
    const { customer_id, install_date } = req.body;
    try {
      var order;
      await OrderTracking.findOne({ id: customer_id })
        .then((result) => {
          if (result) {
      ...
      if (status === "Y") {
        orderRes = await OrderTracking.updateOne(
          { id: order.id },
          {
      ...
      if (!orderRes) {
        console.log(
          "Order Date Update Failed: DB Sales Details persist isssue"
        );
        return res
          .status(400)
